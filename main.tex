...
We consider a simplified system consisting of a circular cavity of radius $r_0$ containing an already-nucleated bubble, surrounded by a smooth horizontal surface with a single-valued, non-hysteretic contact angle $\phi_0$. That is, the contact angle is independent of whether the three-phase contact line advances or recedes. The fluid is quiescent, so forces are purely hydrostatic, and the system is physically equivalent to a pendant drop. Throughout this paper we refer to it as a bubble, but our results apply equally to pendant drops. We determine the detachment size for pinned and spreading bubbles across the full range of cavity sizes, capillary lengths ($0\le r_0/\lambda<\infty$), and contact angles ($0\le\phi_0\le\pi$).
...
V=\int_0^h\pi r^2\,dz,
\end{equation}
is integrated using the midpoint rule. Each family of solutions in Figure~\ref{figProfile}a gives rise to a curve $h(V)$ in Figure~\ref{figHeight}a. Upon nucleation inside the cavity, the bubble has zero volume and zero height, corresponding to the bottom-left point in Figure~\ref{figHeight}a. As the bubble grows, its height $h$ increases and it remains pinned on the $h(V)$ curve until one of two conditions is met:
\begin{enumerate}
...
\caption{(a)~Apex radius of curvature $R_h$ of pinned bubbles with contact radius $r_0$. The red line shows $R_h$ at detachment. The shaded region $3.219\lambda < r_0 < 3.832\lambda$ and dashed red line indicates where $\phi_0$ reaches $\pi$ and the bubble becomes unstable to asymmetric perturbations. The dotted red line shows the maximum horizontal radius $r_\mathrm{max}$ reached during evolution. (b)~Apex radius of curvature $R_h$ of spreading bubbles with contact angle $\phi_0$. Solid blue line: $R_h$ at detachment; dotted blue line: the maximum horizontal radius $r_\mathrm{max}$ reached during evolution. In (a)~and (b), vertical grey solid lines show evolution of $R_h$ up to detachment, with arrowheads corresponding to the bubbles shown in Figure~\ref{figProfile}.}
\label{figAng}
\end{figure}
...
\caption{(a)~Contact angle $\phi_0$ of pinned bubbles with contact radius $r_0$. The red line is the contact angle $\phi_0$ at detachment, the vertical grey lines and arrows show the evolution of the contact angle $\phi_0$ up to detachment, and the dotted grey line $r_0=3.5\lambda(\phi_0/\pi)^2$. (b)~Contact radius $r_0$ of spreding bubbles with contact angle $\phi_0$. The solid blue line is the contact radius $r_0$ at detachment, and the dotted grey line is the fit $r_0=3.219\lambda(\phi_0/\pi)^2$. In (a)~and (b), solid grey lines and arrows show evolution up to detachment, the shaded range $3.219\lambda < r_0 < 3.832\lambda$ and corresponding dashed red line shows where $\phi_0=\pi$ and detachment occus non-axisymmetrically.}
\label{figAng}
\end{figure}
