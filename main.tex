...
For a bubble attached to a horizontal surface (illustrated in Fig.~\ref{figProfile}) the scale over which surface tension $\sigma$ balances fluid density contrast $\rho_\mathrm{out} - \rho_\mathrm{in}$ and gravity $g$ is the capillary length~\cite{laplace98traite},
\begin{equation}
...
\begin{enumerate}
\item If the contact angle $\phi_0$ falls below the contact angle $\phi_0$ of the system, the contact line unpins and spreads across the surface~\cite{chesters78modes}. The bubble then follows an $h(V)$ curve in Figure~\ref{figHeight}b.
\item If the contact angle $\phi_0$ reaches $\pi$, the interface becomes unstable to non-axisymmetric perturbations~\cite{michael76equilibrium}, the interface peak shifts to one side of the cavity, and the bubble detaches laterally. This occurs for $r_0>3.219\,\lambda$, at the right of Figure~\ref{figHeight}a.
